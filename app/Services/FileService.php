...
class FileService
{
    ...
    public function uploadAvatar(\Illuminate\Http\UploadedFile $avatarFile)
    {
        ...
        $filename = date('YmdHis') . Str::random(20).".".$extension;
    ...
    public function uploadProduks(\Illuminate\Http\UploadedFile $avatarFile)
    {
        // if (config('app.is_heroku')) {
        //     $httpsUrl = cloudinary()->upload($avatarFile->getRealPath())->getSecurePath();
        //     return $httpsUrl;
        ...
        $extension = $avatarFile->extension();
        $filename = date('YmdHis') . Str::random(20).".".$extension;
        $avatarFile->storeAs('public/produks', $filename);
        return $filename;
    }
    public function uploadPembelis(\Illuminate\Http\UploadedFile $avatarFile)
    {
        // if (config('app.is_heroku')) {
        //     $httpsUrl = cloudinary()->upload($avatarFile->getRealPath())->getSecurePath();
        //     return $httpsUrl;
        ...
        $extension = $avatarFile->extension();
        $filename = date('YmdHis') . Str::random(20).".".$extension;
        $avatarFile->storeAs('public/pembelis', $filename);
        return $filename;
    }
}
